...
export interface Recipe {
  ...
  /** depends_on block for the main agent service in the generated compose.
   *  Same dual-shape as `services[].dependsOn`: flat list (defaults to
   *  service_started) OR structured map for `service_healthy` /
   *  `service_completed_successfully` conditions.  Used to wire the main
   *  service to one-shot bootstrap sidecars that must complete first. */
  containerDependsOn?: RecipeDependsOn;
  /** Extra bind volumes for the main agent service, on top of the ones
   *  cook derives from workspace mounts, fleet dataDirs, credentialFiles,
   *  and containerTemplateFiles.  Use case: a directory written by a
   *  bootstrap sidecar that the main service needs to read (e.g.
   *  `./wiki-bootstrap` -> `/bootstrap` so the entrypoint can read the
   *  generated bot password). */
  containerVolumes?: Array<{ source: string; target: string; readOnly?: boolean }>;
}
...
export interface RecipeContainerTemplateFile {
  ...
  template: string;
  mode?: string;
  /** When true, cook leaves `${VAR}` tokens UNSUBSTITUTED in the rendered
   *  file and bind-mounts it at `<inContainer>.tmpl` instead of `<inContainer>`.
   *  The conhost entrypoint runs envsubst at container start to fill values
   *  that only exist at runtime (e.g. a bot password generated by a
   *  bootstrap sidecar after the wiki schema is installed). */
  runtimeRender?: boolean;
  /** When non-empty, cook does cook-time substitution as usual EXCEPT for
   *  the listed variable names — those stay as literal `${NAME}` in the
   *  rendered file.  Implies `runtimeRender: true` (file bind-mounted as
   *  `.tmpl` and the entrypoint envsubsts).  Use this instead of a
   *  full-file `runtimeRender` when most vars are known at cook time and
   *  only a small set (e.g. WIKI_BOT_PASSWORD) is generated at runtime —
   *  POSIX envsubst doesn't understand `${VAR:-default}`, so anything that
   *  needs the default-form must be substituted at cook time. */
  runtimeVars?: string[];
}

/** Compose depends_on shape — flat list (default `service_started`) or
 *  per-dep condition map.  Used by both `services[].dependsOn` and the
 *  top-level `containerDependsOn`. */
export type RecipeDependsOn =
  | string[]
  | Record<string, { condition: RecipeDependsOnCondition }>;

export type RecipeDependsOnCondition =
  | 'service_started'
  | 'service_healthy'
...
export interface RecipeSidecarService {
  ...
  secrets?: string[];
  dependsOn?: RecipeDependsOn;
  /** Override the image's default CMD.  Useful for reusing an upstream
   *  image (e.g. `mediawiki:1.42`) as a one-shot bootstrap container that
   *  runs a custom script instead of the default daemon. */
  command?: string[];
...
/** Validate a depends_on value: either a flat list of service names (each
 *  defaulting to `service_started`) or a map of service-name -> condition.
 *  Used by services[].dependsOn AND the top-level containerDependsOn. */
function validateDependsOn(value: unknown, label: string): void {
  ...
  if (Array.isArray(value)) {
    if (!(value as unknown[]).every((s) => typeof s === 'string' && s)) {
      throw new Error(`${label} must be an array of non-empty strings (or an object map)`);
    }
    return;
  }
  if (typeof value === 'object' && value !== null) {
    for (const [name, entry] of Object.entries(value as Record<string, unknown>)) {
      if (!name) {
        throw new Error(`${label} keys must be non-empty service names`);
      }
      if (!entry || typeof entry !== 'object') {
        throw new Error(`${label}.${name} must be an object with a "condition" field`);
      }
      const e = entry as Record<string, unknown>;
      if (typeof e.condition !== 'string' || !allowedConditions.has(e.condition)) {
        throw new Error(
          `${label}.${name}.condition must be one of: ${Array.from(allowedConditions).join(', ')}`,
        );
      }
    }
    return;
  ...
  throw new Error(
    `${label} must be either an array of service names or an object map ` +
    `{ <svc>: { condition: "service_started" | "service_healthy" | "service_completed_successfully" } }`,
  );
}
...
export function validateRecipe(raw: unknown): Recipe {
  ...
  if (obj.containerTemplateFiles !== undefined) {
    ...
    for (let i = 0; i < obj.containerTemplateFiles.length; i++) {
      ...
      if (tf.runtimeRender !== undefined && typeof tf.runtimeRender !== 'boolean') {
        throw new Error(`containerTemplateFiles[${i}].runtimeRender must be a boolean`);
      }
      if (tf.runtimeVars !== undefined) {
        if (!Array.isArray(tf.runtimeVars) || !(tf.runtimeVars as unknown[]).every((s) => typeof s === 'string' && /^[A-Za-z_][A-Za-z0-9_]*$/.test(s as string))) {
          throw new Error(`containerTemplateFiles[${i}].runtimeVars must be an array of env-var-name strings`);
        }
      }
  ...
  if (obj.containerDependsOn !== undefined) {
    validateDependsOn(obj.containerDependsOn, 'containerDependsOn');
  }

  if (obj.containerVolumes !== undefined) {
    if (!Array.isArray(obj.containerVolumes)) {
      throw new Error('containerVolumes must be an array');
    }
    for (let i = 0; i < obj.containerVolumes.length; i++) {
      const vol = obj.containerVolumes[i] as Record<string, unknown>;
      if (!vol || typeof vol !== 'object') {
        throw new Error(`containerVolumes[${i}] must be an object`);
      }
      if (typeof vol.source !== 'string' || !vol.source) {
        throw new Error(`containerVolumes[${i}].source must be a non-empty string`);
      }
      if (typeof vol.target !== 'string' || !vol.target) {
        throw new Error(`containerVolumes[${i}].target must be a non-empty string`);
      }
      if (vol.readOnly !== undefined && typeof vol.readOnly !== 'boolean') {
        throw new Error(`containerVolumes[${i}].readOnly must be a boolean`);
      }
    }
  }
  ...
  if (obj.services !== undefined) {
    ...
    for (let i = 0; i < obj.services.length; i++) {
      ...
      if (svc.dependsOn !== undefined) {
        validateDependsOn(svc.dependsOn, `services[${i}].dependsOn`);
      }
      if (svc.command !== undefined) {
        if (!Array.isArray(svc.command) || !(svc.command as unknown[]).every((s) => typeof s === 'string')) {
          throw new Error(`services[${i}].command must be an array of strings`);
        }
      }
      ...
      if (svc.templateFiles !== undefined) {
        ...
        for (let t = 0; t < svc.templateFiles.length; t++) {
          ...
          // Accept either an exact-file mount (`source: ./wiki-config/x.conf`)
          // OR a parent-directory mount (`source: ./wiki-bootstrap` covering
          // `./wiki-bootstrap/init.sh`).  The latter is the natural shape for
          // bootstrap-style sidecars where the init script + writable state
          // dir share one bind mount.
          const isCovered = volumeSources.has(normalizedTfPath)
            || Array.from(volumeSources).some((src) => {
              const prefix = src.endsWith('/') ? src : src + '/';
              return normalizedTfPath.startsWith(prefix);
            });
          if (!isCovered) {
            throw new Error(
              ...
              `(either the file directly or a parent directory).`,
            );
          }
        }
      }
